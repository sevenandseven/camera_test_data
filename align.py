...
dframe_data = cv2.imread(r'C:\Users\Administrator\Desktop\camera_test_data\zhixiang\Administrator-depth.png', cv2.IMREAD_UNCHANGED)
frame = cv2.imread(r'C:\Users\Administrator\Desktop\camera_test_data\zhixiang\Administrator-RGB.png', cv2.IMREAD_UNCHANGED)#
...
fx = 970.2671508789062
fy = 970.2671508789062
cx = 477.4331970214844
cy = 210.4376983642578
print(dframe_data.shape)
zz = dframe_data * 0.1
print(zz)

ur = np.array([np.arange(dframe_data.shape[1])])
uh = np.repeat(ur, dframe_data.shape[0], axis=0)
uscx = uh - cx
uscxdz = uscx * zz
xx = uscxdz / fx
print(xx)

vc = np.array([np.arange(dframe_data.shape[0])]).T
vw = np.repeat(vc, dframe_data.shape[1], axis=1)
vscy = vw - cy
vscydz = vscy * zz
yy = vscydz / fy
print(yy)
...
fx = 432.4203796386719
fy = 432.4203796386719
cx = 936.9318237304688
cy = 328.924560546875
...
# dfx = 970.2671508789062 * 960 / 960
# dfy = 970.2671508789062 * 600 / 600
# dcx = 477.4331970214844 * 960 / 960
# dcy = 210.4376983642578 * 600 / 600
dfx = 970.2671508789062
dfy = 970.2671508789062
dcx = 477.4331970214844
dcy = 210.4376983642578
# 深度相机的内参矩阵
RK_dep = np.array([(dfx, 0, dcx), (0, fy, dcy), (0, 0, 1)])
...
# 形状   高 宽  （600 960）
result = np.ones((frame.shape[0], frame.shape[1], 3), dtype=np.uint8)*255
print(result.shape)
for row in range(dframe_data.shape[0]):       # 600
    for col in range(dframe_data.shape[1]):    # 960
        depth_value = dframe_data[row, col] *0.1           # 获取深度值  [行列--高宽]
        #print(depth_value)
        if depth_value != 0 and depth_value != 65535:
            ...
            #print(X,Y, "xy is******************")         # X>Y       X是列   
            # 除以齐次坐标的最后一个元素获得像素值   # // Z_rgb*p_rgb -> p_rgb
            #print(frame.shape[0])        # 1080
